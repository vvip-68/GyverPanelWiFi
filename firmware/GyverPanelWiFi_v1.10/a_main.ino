...
void process() {  
  ...
  if (!parseStarted) {
    ...
    if (init_time && !(wifi_connected && useNtp)) {
      ...
      // нассыет на следующий день не срабатывает, т.к. рассчитан неправильно
      bool timeToSync = ntpSyncTimer.isReady();
    ...
    checkAutoMode6Time();
      ...
      // Устройство с включенным MQTT периодически теряет связь в т.ч и через локальную точку доступа и начинает жутко тормозить
      // вероятно проблемы внутри ядра ESP32 и/или библиотеки поддержки MQTT.
      // Не нашел другого способа исправления "зависшего" устройства кроме ежедневной автоматической перезагрузки в указанное в REBOOT_HOUR время.
      // За час до назначенного времени устанавливается флаг необъодимости перезагрузки.
      // В назначенный час если флаг установлен - выполняется перезапуск микроконтроллера
      // После перещзапуска флаг оказывается сброшен - повторной перезагрузки в указанный час выполнено не будет.
      // Через сутки за час до назначенного времени флаг будет устиановлено - перезагрузка снова выполнится в указанное время.
      if (init_time) {
        int8_t hrs, chk_hour;
        chk_hour = REBOOT_HOUR - 1;
        if (chk_hour < 0) chk_hour = 23;
        hrs = hour();        
        if (!need_reboot && hrs == chk_hour) {
          // Взвести "курок" необходимости перезагрузки за час до назначенного времени
          need_reboot = true;
        }        
        // Если "курок" взведен и настало время перезагрузки - перезагрузить микроконтроллер
        if (need_reboot && hrs == REBOOT_HOUR) {
          ESP.restart();
        }
      }
    #endif

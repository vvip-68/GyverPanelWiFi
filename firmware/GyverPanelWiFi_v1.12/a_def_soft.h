...
#else
#define MAX_EFFECT              42         // количество эффектов, определенных в прошивке
#endif
...
uint8_t  nightClockBrightness = 2;          // Яркость отображения ночных часов
...
uint32_t nextMqttConnectTime;               // При недоступности MQTT ервера определяет время следующей попытки соединения
#endif
...
uint8_t  weatherActualityDuration = 3;      // Какой период времени в часах после получения погоды считать ее актуальной (на случай, если сервер перестал отвечать)
...
uint32_t idleTime = ((uint32_t)IDLE_TIME * 60 * 1000UL);  // минуты -> миллисек
uint32_t autoplayTime = ((uint32_t)AUTOPLAY_PERIOD * 1000UL); // секунды -> миллисек
...
uint8_t    sMATRIX_TYPE = MATRIX_TYPE;           // тип соединения диодов в сегменте матрицы: 0 - зигзаг, 1 - параллельная

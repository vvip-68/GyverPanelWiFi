...
// При этом если флаг EEPROM_OK в строке 8 этого файла был изменен и не совпадает со считанными яз ячейки #0 значением -
...
#define EFFECT_LIST F("Часы,Лампа,Снегопад,Кубик,Радуга,Пейнтбол,Огонь,The Matrix,Шарики,Звездопад,Конфетти," \
                      ...
                      "Палитра,Спектрум,Синусы,Вышиванка,Дождь,Камин,Стрелки,Погода,Жизнь,Узоры,Рассвет,SD-Карта")
#else
#define EFFECT_LIST F("Часы,Лампа,Снегопад,Кубик,Радуга,Пейнтбол,Огонь,The Matrix,Шарики,Звездопад,Конфетти," \
                      "Цветной шум,Облака,Лава,Плазма,Радужные переливы,Павлин,Зебра,Шумящий лес,Морской прибой,Смена цвета," \
                      "Светлячки,Водоворот,Циклон,Мерцание,Северное сияние,Тени,Лабиринт,Змейка,Тетрис,Арканоид," \
                      "Палитра,Спектрум,Синусы,Вышиванка,Дождь,Камин,Стрелки,Погода,Жизнь,Узоры,Рассвет")
#endif
...
#define EFFECT_LIST F("Часы,Лампа,Снегопад,Кубик,Радуга,Пейнтбол,Огонь,The Matrix,Шарики,Звездопад,Конфетти," \
                      "Цветной шум,Облака,Лава,Плазма,Радужные переливы,Павлин,Зебра,Шумящий лес,Морской прибой,Смена цвета," \
                      "Светлячки,Водоворот,Циклон,Мерцание,Северное сияние,Тени,Лабиринт,Змейка,Тетрис,Арканоид," \
                      "Палитра,Спектрум,Синусы,Вышиванка,Дождь,Камин,Стрелки,Анимация,Погода,Жизнь,Узоры,Рассвет,SD-Карта")
#else
#define EFFECT_LIST F("Часы,Лампа,Снегопад,Кубик,Радуга,Пейнтбол,Огонь,The Matrix,Шарики,Звездопад,Конфетти," \
                      "Цветной шум,Облака,Лава,Плазма,Радужные переливы,Павлин,Зебра,Шумящий лес,Морской прибой,Смена цвета," \
                      "Светлячки,Водоворот,Циклон,Мерцание,Северное сияние,Тени,Лабиринт,Змейка,Тетрис,Арканоид," \
                      "Палитра,Спектрум,Синусы,Вышиванка,Дождь,Камин,Стрелки,Анимация,Погода,Жизнь,Узоры,Рассвет")
#endif
...
#define MC_WEATHER              38
#define MC_LIFE                 39
#define MC_PATTERNS             40
#define MC_DAWN_ALARM           41
#define MC_SDCARD               42

#if (USE_SD == 1)
#define MAX_EFFECT              43         // количество эффектов, определенных в прошивке
#else
#define MAX_EFFECT              43         // количество эффектов, определенных в прошивке
#endif
...
#define D_CLOCK_SPEED 255                 // скорость перемещения эффекта часов по умолчанию (мс) - если больше 240 - часы стоят, не прокручиваются
...
String ssid       = NETWORK_SSID;           // SSID (имя) вашего роутера (конфигурируется подключением через точку доступа и сохранением в EEPROM)
...
// Режимы работы панели - Саcомтоятельный без поддержки синхронизации, Мастер (ведущий), Слушатель (подчиненный)
enum  eWorkModes   {
  STANDALONE, 
  MASTER, 
  SLAVE
...
uint8_t    syncGroup = 0;                   // Номер группы синхронизации (одна группа может содержать до 7 вселенных (по протоколу управления E1.31 DMX MULTICAST), т.е для Группы 0 - 1..7 вселенные, для Группы 1 - 8..14 вселенные и т.д.
...
#define    E131_FPS_INTERVAL  10000         // Отображать FPS получаемого потока каждые 10000 мсек (10 сек); Установите 0  если отображать не нужно; 
...
byte       sWIDTH = WIDTH;                       // ширина одного сегмента матрицы
byte       sHEIGHT = HEIGHT;                     // высота одного сегмента матрицы
byte       sMATRIX_TYPE = MATRIX_TYPE;           // тип соединения диодов в сегменте матрицы: 0 - зигзаг, 1 - параллельная
byte       sCONNECTION_ANGLE = CONNECTION_ANGLE; // угол подключения диодов в сегменте: 0 - левый нижний, 1 - левый верхний, 2 - правый верхний, 3 - правый нижний
byte       sSTRIP_DIRECTION = STRIP_DIRECTION;   // направление ленты из угла сегмента: 0 - вправо, 1 - вверх, 2 - влево, 3 - вниз

byte       mWIDTH = META_MATRIX_WIDTH;           // количество сегментов в ширину
byte       mHEIGHT = META_MATRIX_HEIGHT;         // количество сегментов в высоту
byte       mTYPE = META_MATRIX_TYPE;             // соединение сегментов мета-матрицы: 0 - зигзаг, 1 - параллельная
byte       mANGLE = META_MATRIX_ANGLE;           // угол 1-го сегмента мета-матрицы: 0 - левый нижний, 1 - левый верхний, 2 - правый верхний, 3 - правый нижний
byte       mDIRECTION = META_MATRIX_DIRECTION;   // направление следующих сегментов мета-матрицы из угла: 0 - вправо, 1 - вверх, 2 - влево, 3 - вниз

uint16_t   pWIDTH = sWIDTH * mWIDTH;
uint16_t   pHEIGHT = sHEIGHT * mHEIGHT;

uint32_t   NUM_LEDS = pWIDTH * pHEIGHT;
uint16_t   maxDim   = max(pWIDTH, pHEIGHT);
uint16_t   minDim   = min(pWIDTH, pHEIGHT);

...
/*
 * Wemos d1 mini
 * В менеджере плат выбрано Wemos d1 mini (clone)
 * или
 * ModeMCU V3
 * В менеджере плат выбрано NodeMCU 1.0 (ESP-12E)
 */ 
...
#define DIO D5                  // D5 TM1637 display DIO pin - требуется для компиляции скетча (USE_TM1637 == 1) // !!! USE_SD == 0 - иначе несовместимость по пинам
#define CLK D7                  // D7 TM1637 display CLK pin - требуется для компиляции скетча (USE_TM1637 == 1) // !!! USE_SD == 0 - иначе несовместимость по пинам
...
#include <ArduinoJson.h>         // Библиотека для работы с JSON (погода, mqtt, состояние системы) -  версия не старше 6.21.5 (!!!)

...
void FastLEDshow() {
  ...
  if (millis() - prevTimer > 5) {
     ...
     // Почему-то на ESP8266 на версиях ядра 3.x.x часто вместо вывода на матрицу просто мерцает первый светодиод
     // В issues FastLED пишут что это случается у многих. Объяснений нет, но говорят часто помогает
     // пнуть вывод на матрицу второй раз сразу же после первого

...
#define FIRMWARE_VER F("WiFiPanel v.1.13.2024.1217")
...
// 
// Альтернативные варианты для ESP8266
// Версия ядра ESP8266 - 2.7.4  // Наиболее стабильная по выводу изображения на матрицу без артефактов и подергиваний   
// Версия FastLED      - 3.5.0  // 3.6.0 / 3.7.0  - смотри комментарий ниже в секции *** FastLED
//
// Для версии ядра ESP8266 3.1.2
...
//
// Для версии ядра ESP8266 2.7.4
//   Плату выбираем реально которую используете - для NodeMCU - "NodeMCU 1.0 (ESP12E Module)", для Wemos d1 mini - "LOLIN(WEMOS) D1 R2 & mini"
//   Пин вывода на ленту в конфигурации по умолчанию LED_PIN обозначен как D2 (GPIO4) - вывод идет на D2
//
...
// *** FastLED
// 
// ESP8266 - 3.1.2, ESP32 - 2.0.14, библиотеки FastLED версий 3.6.0 или 3.7.0
// ESP8266 - 2.7.4, библиотеки FastLED версий 3.5.0 или 3.6.0 или 3.7.0 
// На последней на текущий момент версии 3.9.4 - проект не собирается - слишком много изменений
//
// -------------------------------------------------------------------------------------------------------
//
// *** ArduinoJSON
...
// На ESP8266 и контроллерах ESP32 с одним ядром эти процессы могут мешать друг други. В результате - либо мерцающий светодиод, либо проблемы подключения к сети.
...
//    Если не помогает - попробуйте последовательно увеличивать это значение 200-250-300-350, каждый раз пересобирая проект.
...
//
// 4. Выбрать вариант сборки проекта - ядро 2.7.4, FastLED - 3.5.0 - на этой комбинации описанной проблемы еще ни укого не возникало
//
//
//
// *******************************************************************************************************
    ...
    while (!(stop_waiting || wifi_connected)) {
      ...
      if (millis() - last_wifi_check > 100) {
        ...
        if (wifi_connected) {
          ...
          #ifndef ARDUINO_ESP8266_RELEASE_2_7_4
          DEBUG(F(", hostname: "));

...
void doEffectWithOverlay(byte aMode) {
  ...
  bool textOvEn  = ((textOverlayEnabled && (getEffectTextOverlayUsage(aMode))) || ignoreTextOverlaySettingforEffect) && !isTurnedOff && !isNightClock && thisMode != MC_CLOCK && thisMode < SPECIAL_EFFECTS_START;
  ...
  if (clockReady) {
    byte cw = WIDTH == 25 ? 25 : 26;                                        // Если ширина матрицы 25 колонок - юрать ширину часов 25, если больше - 26. Отрисовка точек между часами и минутами - одинарные или сдвоенные
  ...
  if (idleState) {
    ...
    if ((ms - autoplayTimer > autoplayTime) && !manualMode) {    // таймер смены режима
      ...
      // (thisMode == MC_SNAKE    && !gameOverFlag) ||   // Змейка долгая игра - не нужно дожидаться окончания, можно прервать
         ...
         (thisMode == MC_ARKANOID && !gameOverFlag) ||   // Арканотд не меняем на другой эффект, пока игра не закончится

  ...
  if (!parseStarted) {
    ...
    if (wifi_connected) {
      ...
      if (useNtp) {
        ...
        // Это помогжет избежать диких тормозов прошивки.        
...
// Если ширина матрицы меньше чем ее высота - передача будет происходить по строкам сверху вниз
  ...
  /*
    ...
    6 - текст $6 N|some text, где N - назначение текста;
       ...
       12 - картинка п околонкам $6 12|X colorHEX Y|colorHEX Y|...|colorHEX Y;   - пока не реализовано (зарезервировано)
    8 - эффект
    ...
    switch (intData[0]) {
      ...
      case 18: 
        if (intData[1] == 0) { // ping
          ...
          // Для команд, прешедших от MQTT отправлять ответы так же как и для комант, полученных из UDP

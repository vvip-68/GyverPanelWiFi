...
// Внимание!!! Если вы меняете эти значения ПОСЛЕ того, как прошивка уже хотя бы раз была загружена в плату и выполнялась,
// чтобы изменения вступили в силу нужно также изменить значение константы EEPROM_OK в первой строке в файле eeprom.ino 
#define  DEFAULT_MQTT_SERVER "m15.cloudmqtt.com" // MQTT сервер по умолчанию
...
#define  TOPIC_CMD "panel/$/cmd"         // Топик - получение команды управления от клиента         '$' заменяется на DEVICE_ID
#define  TOPIC_DTA "panel/$/dta"         // Топик - отправка запрошенных данных клиенту             '$' заменяется на DEVICE_ID
#define  TOPIC_NFO "panel/$/nfo"         // Топик - отправка информационных уведомлений клиенту     '$' заменяется на DEVICE_ID
#define  TOPIC_ERR "panel/$/err"         // Топик - отправка уведомлений об ошибке клиенту          '$' заменяется на DEVICE_ID
#define  TOPIC_ACK "panel/$/ack"         // Топик - отправка уведомлений клиенту                    '$' заменяется на DEVICE_ID
...
uint16_t mqtt_device_id  = DEVICE_ID;    // ID устройства как часть топика
...
#define QSIZE 16                         // размер очереди команд от MQTT
String  cmdQueue[QSIZE];                 // Кольцевой буфер очереди полученных команд от MQTT
String  outQueue[QSIZE];                 // Кольцевой буфер очереди отправки команд в MQTT (message)
String  tpcQueue[QSIZE];                 // Кольцевой буфер очереди отправки команд в MQTT (topic)

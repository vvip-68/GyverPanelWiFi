...
#define CAL_Y      (byte((HEIGHT - (2*5 + 1)) / 2.0) + 1)          // Две строки цифр 5 пикс высотой + 1 пробел между строкми / 2 - в центр
...
boolean  textHasDateTime = false;        // Строка содержит макрос отображения текущего времени - ее нужно пересчитывать каждый раз при отрисовкеж Если макроса времени нет - рассчитать текст строки один раз на этапе инициализации
...
WiFiUDP udp;                             // Объект ранспорта сетевых пакетов
WiFiClient client;                       // Объект для работы с удалёнными хостами
...
PubSubClient mqtt(client);               // Объект соединения с MQTT сервером
...
// Внимание!!! Если вы меняете эти значения ПОСЛЕ того, как прошивка уже хотя бы раз была загружена в плату и выполнялась,
// чтобы изменения вступили в силу нужно также изменить значение константы EEPROM_OK в первой строке в файле eeprom.ino 
#define  DEFAULT_MQTT_SERVER "mqtt-serv" // MQTT сервер по умолчанию
#define  DEFAULT_MQTT_USER "mqtt-user"   // Имя mqtt-пользователя по умолчанию 
#define  DEFAULT_MQTT_PASS "mqtt-pass"   // Пароль mqtt-пользователя по умолчанию
#define  DEFAULT_MQTT_PORT 14057         // Порт mqtt-соединения по умолчанию
#define  TOPIC_PREFIX "mqtt-user/"       // Некоторые сервера требуют обязательный префикс в виде имени пользователя в топике публикации / подписки
                                         // то есть, если имя пользователя "mqtt-user" - топик отправки команды управления должен иметь вид:  "mqtt-user/panel/id/cmd"
                                         // Если сервер не требует обязательного префикса - оставьте TOPIC_PREFIX пустым и тот же топик будет иметь вид "panel/id/cmd"
                                         // здесь "id" - ID вашего устройства как часть топика - задается дефайном DEVICE_ID, значение которого будет подставлено
                                         // на место символа '$' в топиках, определенных ниже                                                                                  
#endif
...
#define  MQTT_SEND_DELAY 1000            // Отправлять сообщение на MQTT-сервер не чаще 1 сообщения в секунду (ограничение бесплатного MQTT сервера)ж Если нет ограничений - поставьте здесь 0
#define  TOPIC_CMD      "cmd"            // Топик - получение команды управления от клиента         '$' заменяется на DEVICE_ID
#define  TOPIC_DTA      "dta"            // Топик - отправка запрошенных данных клиенту             '$' заменяется на DEVICE_ID
#define  TOPIC_NFO      "nfo"            // Топик - отправка информационных уведомлений клиенту     '$' заменяется на DEVICE_ID
#define  TOPIC_ERR      "err"            // Топик - отправка уведомлений об ошибке клиенту          '$' заменяется на DEVICE_ID
#define  TOPIC_ACK      "ack"            // Топик - отправка уведомлений клиенту                    '$' заменяется на DEVICE_ID
...
String  topic_nfo = TOPIC_NFO;           // Топик отправки уведомдений
...
bool    useMQTT = true;                  // Использовать канал управления через MQTT
bool    mqtt_connecting = false;         // Выполняется подключение к MQTT (еще не установлено)
byte    mqtt_conn_cnt = 0;               // Счетчик попыток подключения для форматировния вывода
...
boolean  init_weather = false;           // Флаг: true - погода полученаж false - погода не получена / не актуальна

#if (USE_WEATHER == 1)
uint32_t regionID = 62;                  // Код региона по Yandex для выбора часового пояса https://tech.yandex.ru/xml/doc/dg/reference/regions-docpage/
boolean  useWeather = true;              // Использовать получение текущей погоды с погодного сервера
String   sunriseTime, sunsetTime;        // Время рассвета/заката в формате HH:mm
...
bool     overlayDelayed2 = false;        // используется для указания был ли оверлей ТЕМПЕРАТУРЫ на пердыдцщем цикле отрисовки

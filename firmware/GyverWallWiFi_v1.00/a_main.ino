...
void parsing() {
  ...
  /*
    ...
    8 - эффект
      - $8 0 номер эффекта;
      - $8 1 N D1 D2; D1 -> параметр #1 для эффекта N; D2 -> параметр #2 для эффекта N
  ...
  */  
  ...
  if (recievedFlag && intData[0] > 0 && intData[0] <= 23) {
    ...
    switch (intData[0]) {
      ...
      case 8:      
        tmp_eff = intData[2];        
        // intData[1] : дейстие -> 0 - выбор эффекта;  1 - параметрs #1 и #2; 2 - вкл/выкл "использовать в демо-режиме"
        // intData[2] : номер эффекта
        // intData[3] : действие = 1: значение параметра #1
        //              действие = 2: 0 - выкл; 1 - вкл;
        // intData[4] : действие = 1: значение параметра #2
        if (intData[1] == 0) {          
          ...
          setScaleForEffect2(tmp_eff,   intData[4]);
          effectScaleParam  [tmp_eff] = intData[3];
          effectScaleParam2 [tmp_eff] = intData[4];
...
void sendPageParams(int page) {
  ...
  switch (page) { 
    ...
    case 2:  // Эффекты. Вернуть: Номер эффекта, Остановлен или играет; Яркость; Скорость эффекта; Использовать в демо 
      ...
      str+="|SQ:"+(thisMode == MC_RAINBOW_DIAG
         ? String(effectScaleParam2[thisMode])
         : "X"); 
      str+=";";
      break;

...
byte   CLOCK_X = CLOCK_X_H1;              // По умолчанию - 3x5, горизонтальные - позиция вывода по X
byte   CLOCK_Y = CLOCK_Y_H1;              // По умолчанию - 3x5, горизонтальные - позиция вывода по Y

int8_t CLOCK_XC = CLOCK_X;                // Текущая позиция часов оверлея по оси Х 
byte   CLOCK_MOVE_DIVIDER = 10;           // Делитель такта таймера эффектов для движения часов оверлея (режим прокручивания часов по горизонтали)
...
CRGB       overlayLEDs[WIDTH * LET_HEIGHT];    // буфер оверлея; по максимуму - для бегущей строки во всю ширину матрицы

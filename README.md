...
При питающем напряжении с источника питания 4.0-4.8 вольт включение диода не обязательно, т.к уровни логических сигналов на пинах мкроконтроллера и DIN ленты согласуюьтся достаточно хорошо.  
...
что в результате повышает уровень логической 1 на выходных пинах микроконтроллера до 3.3+0.6 ~= 3.8-3.9 вольта, которые матрица уверенно распознает даже при повышении
питания до 5.5-6 вольт. 

## Управление питанием
Матрица, даже если она "выключена" - ничего не отображает все равно потребляет довользо значительный ток от источника питания.
...
Управление питанием реализуется следующей схемой:

![SCHEME](https://github.com/vvip-68/GyverPanelWiFi/blob/master/schemes/mosfet.jpg)

Используется Р-канальный мосфет IFR4905. Оптрон pc817 - гальваническая развязка и защита пина ESP, а также обеспечивает управление уровнем 3.3 вольта
с которым работает микроконтроллер. Мосфет лучше брать в корпусе TO-220. IFR4905 обеспечивает ток питания матрицы 74А при 25 градусах, 52А - при 100 градусах.
Управляющий сигнал POWER_ON в HIGH. 

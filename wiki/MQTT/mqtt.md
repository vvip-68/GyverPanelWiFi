...
Как правило сервера отличаются наличием наряду с платными бесплатных сервисов, которые, однако, имеют ограничение на количество одновлеменных подключений к брокеру,
количеством разрешенных запросов в период времени, наличием / отсутствием авторизации при подключении, поддержкой SSL и так далее.
...
Открывшаяся страница содержит сведения о параметрах подключения, ожибаемых брокером.  
...
1. Сервер - имя сервераЮ к которому будет выполняться подключение для создания MQTT-канала управления
...
Этих сведений достаточно для настройки соединения в прошивке.  
...
Перенесите значения соответствующих настроек из окна "Профиль" настроек подключения в прошивку  
...
значением DEVICE_ID, объявленным в файле *"a_def_hard.h"* в строке 14. В моем проекте здаесть просто список работающих у меня устройств и определяется конкретный
...
характеризует и идентифицирует устройство - он и испоользуется для создания идентификатора устройства на сервере MQTT.  

Итак, идентификатор клиента в этом примере - "WiFiPanel-0"? поскольку создается как: MQTT_CLIENT_ID + "-" + DEVICE_ID  
...
Введите в поле идентификатор клиента (в нашем случае WiFiPanel-0) и нажмите кнопку "Добавить". Теперь сервер знает, что кнему может подключаться 
наше собранное на ESP8266 устройство  

Для управления по каналу MQTT требуется как минимум 2 участника - наше устройства и некоторый менеджер, который будет отправлять управляющие команды устройству.
Это может быть соответствующая программа на телефоне или же панель консоли MQTT-брокера, например [эта](client.mqtt.4api.ru). 
...
Тем не менее данный Web-client ограничен в своих возможностях и мало на что годиться. 
...
Следующий шаг - регистрация топиков, которые брокер ожидает от устройства. Нажмите кнопку "Фвв New Topic Suscription". 
...
и отправить команду в устройство, нажав кнопку "Publish". Реакцию устройства на команду можно видеть в мониторе порта и в секции "Messages" панели управления  
...
Если у вас есть соответствующие знания - вы можете настроить управление устройством через голосовые помошники, например - Яндекс Алиса или Google Assistant.

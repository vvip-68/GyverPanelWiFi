...
> **ВНИМАНИЕ!** В связи с изменением у брокера [mqtt.by](https://mqtt.by/) формата префикса, в приведенных ниже примерах везде вместо строки ***"af7cd12a"*** требуется использовать ***"user/<имя пользователя>/af7cd12a"***. Так же требуется задать имя пользователя и пароль.
Подробности настройки подключения см. на странице брокера **"Профиль"**.

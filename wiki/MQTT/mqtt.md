...
количество разрешенных запросов в период времени, наличие / отсутствие авторизации при подключении, поддержкy SSL и так далее. Выбор конкретного брокера 
зависит от ваших предпочтений и требований по защите канала (авторизация, наличие SSL и т.д). 
Вместе с тем следует принимать во внимание ряд параметров от которых зависит будет ли вообще устройство нормально работать с управлением по MQTT-кналу.

### Обработка пакета сообщений

В процессе коммуникации устройство отправляет вновь подключившимуся клиенту одновременно большое количество сообщений (около 80), содержащих
...
После физического выключения устройства от сети или потери устройством связи с MQTT-сервером, брокер автоматически рассылает
...
Для пробного упрощенного использования управления пок каналу MQTT в проекте используется бесплатный 
общедоступный брокер для интернета вещей - [mqtt.iotwithus.com](https://www.iotwithus.com/free-mqtt-server/). 
Данный MQTT-брокер не использует авторизацию по логину и паролю, имеет простые настройки и удовлетворяет описанным выше необъодимым требованиям.
Передача сообщений для конкретного устройства обеспечиваются уникальностью топика сообщения - задайте для вашего устройства 
гарантированный уникальный префикс, который с большой долей вероятности не будет пересекаться с префиксами топиков других пользователей.
Например, используйте для префикса набор символов, формируемый из [GUID](https://ru.wikipedia.org/wiki/GUID) - Globally Unique Identifier, 
...
вашего устройства, его ID или придумайте набор символов сами. Длина сформированного префикса не должа превышать 30 символов.
...
Откройте в Arduino IDE файл прошивки **"a_def_soft.h"**. Найтите строки 325-343.  
Строки указаны для версии скетча WiFiPanel-v.1.09.2021.0216. Если у вас другая версия - найдите строки, соответствующие строкам на картинке.  
...
задаются непосредстванно в строках 325-343 вкетча в файле **"a_def_soft.h"**.  
...
Настройки соединения с MQTT-сервером, сделанные в скетче по описанию выше, используются как значения по-умолчанию и сохраняются в энергонезависимой памяти
при первой загрузке прошивки в микроконтроллер. При дальнейших запусках микроконтроллера настройки считываются из EEPROM и используются
...
Для того, чтобы приложение на смартфоне могло подключаться к устройству черкз MQTT-канал, требуется сконфигурировать параметры соединения.
...
После этого откроется панель редактирования параметров подключения.

![Fast Patch](https://github.com/vvip-68/GyverPanelWiFi/blob/master/wiki/MQTT/mqtt-17.png)  

Панель содержит следующие поля:

1. `Имя` - наименование подключения, как вы его видете в комбобоксе выбора на предыдущем изображении.  
...
4. `Порт` - порт родключения по протоколу UDP. Значение по умолчанию - 2390, задается в скетче константой

...
- Комбобокс **"Вариант"** позволяет выбрать один из возможных вариантов эффекта.  
  ...
  Как правило, один из вариантов - ***"Случайный"***. Выбор этого варианта позволяет устройству выбирать выриант случайным образом.  
...
- Чекбокс **"Сегменты"** - переключение между широкоформатным видом эффекта или видом разделенным на сегменты.  
  ...
  Так, матрица 48x16 может быть разделена на три квадратных сегмента. Сам эффект рисуется синхронно в каждом сегменте.
...
- **"Интервал"** - задается время в секундах - период отображения. Очередной текст будет отображен поверх эффектов с разрешенным 
...
**Ячейки хранения текста**. Каждой ячейке соответствует кнопка от 0 до 9, а так же A..Z. Нажатие на кнопку загружает в строку редактирования 
текст из соответствующей ячейки. Введенный в строку текст можно сохранить, нажав на кнопку **"Сохранить"**. Нажатие на кнопку **"Просмотр"**
помимо сохранения текста в устройстве также запускает отображение бегущей строки на устройстве.  
...
- Чекбокс **"Синхронизация NTP"** позволяет автоматически получать текущее время и серверов времени, расположенных в сети Интернет.  
...
- Варианты отображения температуры - **"Цветная температура"**  
  ...
  Если отображение цветов температуры включено:  
   - отрицательные температуры до -4° отображаются цветами синих оттенков. Чем холоднее - тем более глубокий синий цвет.  
   - температура в диапазоне -3°..+3° отображается белым цветом  
   - положительные температуры от +4° отображаются цветами красных оттенков. Чем теплее - тем более насыщенный красный цвет.  
...
- Чекбокс **"Часы поверх эффектов"** - отключение чекбокса прекращает отображение часов во всех эффектах.
  При включенном чекбоксе часы будут показываться поверх тех эффектов, в которых разрешего их отображение.  
...
### Настройки будильников
  ...
  Особенностью будильников является возможность включения в режиме "рассвет", когда за несколько минут до срабатывания основного будильника
  включается плавное рарастантие яркости матрицы, имитируя наступление рассвета.  
...
- Чекбокс **дня недели** и **выбор времени** будильника. Для каждого дня недели - свой чекбокс и поле выбора времени.
...
![Приложение-15](https://github.com/vvip-68/GyverPanelWiFi/blob/master/wiki/Tune/p16-3.png)
  ...
  При этом во время рассвета громкость звука будет автоматически увеличиваться от минимальной до установленной громкости,
...
- **"Выключить матрицу"** - в указанное время произойдет программное отключение устройство. Питание с матрицы бедет снято (при наличии модулей управления питанием 
...
Наличие этой информации позволяет запрограммировать включение гирлянды с наступлением сумерек (***Закат***) и выключить гирлянду с началом нового дня - утром (***Рассвет***).

...
Одна вселенная вмещает данные RGB-цветов для 170 светодиодов (170 * 5 = 510 каналов).  
...
Инструмент **"Fast Patch"** использует лоя разметки прямоугольные области, внутри которых задает последовательность диодов согласно выбранным настройкам. После завершения распределения диодов для первой вселенной выполним разметку для второй вселенной.
...
Прямоугольная область разметки остатка строки - 6 клетов в одной строке - укажите размер заполняемой области в поле  **"Dimension"** - **X:6, Y:1**.
...
Для остановке трансляции повторно выберите этот же пункт меню.  

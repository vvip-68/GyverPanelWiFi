...
- **{Rdate time#N}** оставшееся до события время, где  
    ...
    **time** - время события в формате ЧЧ:ММ  
               Время может быть опущено, тогда принимается 00:00  
    **N**    - индекс строки (1..9,A..Z или 1..35) отображаемой ПОСЛЕ наступления события.  
               Если N не указана - после наступления события cтрока отображаться не будет.  
...
- **{Sdate1 time1#date2 time2}** период отображения строки, где  
    ...
    **time1** - время начала периода в формате ЧЧ:ММ  
    **date2** - дата окончания периода в формате ДД.ММ.ГГГГ  
    **time2** - время окончания периода в формате ЧЧ:ММ  
     ...
     - если дата начала периода не указано - означает "каждый день"  
     - если дата окончания периода не указано - та же дата, что дата начала  
     - если время начала периода не указано - означает 0:00  
     - если время окончания периода не указано - означает 23:59  
